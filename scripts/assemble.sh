...
CONFIG_PATH="/tsl/scratch/witekk/masurca_3.2.3_assembly_May23/masurca_config.txt"
CMD_PATH="/tsl/software/testing/bin/core/../../masurca/3.2.3/x86_64/bin/masurca"
...
export PATH="/tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/bin:/tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/bin/../CA/Linux-amd64/bin:$PATH"
save PATH
export PERL5LIB=/tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=320
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'pe 250 20' >> meanAndStdevByPrefix.pe.txt
#rename_filter_fastq 'pe' <(exec expand_fastq '/tsl/scratch/witekk/nanopore/left.fq.gz' | awk '{if(length($0>200)) print substr($0,1,200); else print $0;}') <(exec expand_fastq '/tsl/scratch/witekk/nanopore/right.fq.gz' | awk '{if(length($0>200)) print substr($0,1,200); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>3300000000)printf "%.0f",s;else print "3300000000";}'`
save JF_SIZE
perl -e '{if(int('$JF_SIZE')>3300000000){print "WARNING: JF_SIZE set too low, increasing JF_SIZE to at least '$JF_SIZE', this automatic increase may be not enough!\n"}}'
log Creating mer database for Quorum
#quorum_create_database -t 320 -s $JF_SIZE -b 7 -m 24 -q $((MIN_Q_CHAR + 5)) -o quorum_mer_db.jf.tmp pe.renamed.fastq && mv quorum_mer_db.jf.tmp quorum_mer_db.jf
if [ 0 != 0 ]; then
...
#quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 320 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor --verbose 1>quorum.err 2>&1 || {
...
#jellyfish count -m 31 -t 320 -C -s $JF_SIZE -o k_u_hash_0 pe.cor.fa
export ESTIMATED_GENOME_SIZE=`jellyfish histo -t 320 -h 1 k_u_hash_0 | tail -n 1 |awk '{print $2}'`
...
create_k_unitigs_large_k -c $(($KMER-1)) -t 320 -m $KMER -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER -f `perl -e 'print 1/'$KMER'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.fasta
...
create_k_unitigs_large_k -c $(($KMER_J-1)) -t 320 -m $KMER_J -n $(($ESTIMATED_GENOME_SIZE*2)) -l $KMER_J -f `perl -e 'print 1/'$KMER_J'/1e5'` pe.cor.fa  | grep --text -v '^>' | perl -ane '{$seq=$F[0]; $F[0]=~tr/ACTGactg/TGACtgac/;$revseq=reverse($F[0]); $h{($seq ge $revseq)?$seq:$revseq}=1;}END{$n=0;foreach $k(keys %h){print ">",$n++," length:",length($k),"\n$k\n"}}' > guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp && mv guillaumeKUnitigsAtLeast32bases_all.jump.fasta.tmp guillaumeKUnitigsAtLeast32bases_all.jump.fasta 
fi
...
rm -rf work1
CA_DIR="CA";
createSuperReadsForDirectory.perl -doSimpleMerge -l $KMER -mean-and-stdev-by-prefix-file meanAndStdevByPrefix.pe.txt -kunitigsfile guillaumeKUnitigsAtLeast32bases_all.fasta -t 320 -mikedebug work1 pe.cor.fa 1> super1.err 2>&1
/tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/bin/mega_reads_assemble_nomatch.sh -t 320 -e $ESTIMATED_GENOME_SIZE -m work1 -p /tsl/scratch/witekk/nanopore/high_quality_minion_data.fastq -a /tsl/software/testing/brew/default/x86_64/Cellar/masurca/3.2.3/CA8/Linux-amd64/bin/  -o "   cgwErrorRate=0.12 useGrid=0 scriptOnGrid=0 merylThreads=16 frgCorrThreads=1 frgCorrConcurrency=12 cnsConcurrency=6 ovlCorrConcurrency=10 ovlConcurrency=10 ovlThreads=8 ovlMemory=8GB" -B 13 -D 0.02
PE_AVG_READ_LENGTH=`awk '{if(length($1)>31){n+=length($1);m++;}}END{print int(n/m)}' pe_data.tmp`
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing.'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 3300000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 320 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
...
if [ -s $CA_DIR/$TERMINATOR/genome.scf.fasta ];then
  if [ ! -e $CA_DIR/filter_map.contigs.success ];then
    ...
    deduplicate_contigs.sh $CA_DIR genome 320 $PLOIDY $TERMINATOR && echo "Assembly complete, final scaffold sequences are in $CA_DIR/dedup.genome.scf.fasta"
  else
  log "Assembly complete, final scaffold sequences are in $CA_DIR/dedup.genome.scf.fasta"
  fi
else
  fail "Assembly stopped or failed, see $CA_DIR.log"
fi
